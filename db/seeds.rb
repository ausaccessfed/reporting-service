...
$stderr.puts warning unless ENV['AAF_DEV'].to_i == 1

if ENV['AAF_DEV'].to_i == 1
  ...
  ActiveRecord::Base.transaction do
    ...
    idps = (1..70).map { create(:identity_provider, organization: orgs.sample) }

    sps = (1..70).map { create(:service_provider, organization: orgs.sample) }

    rapid_services = (1..90).map { create(:rapid_connect_service, organization: orgs.sample) }

    [*idps, *sps, *rapid_services, *orgs].each do |object|
      next if rand > 0.9

      old_activation = create(:activation, :deactivated, federation_object: object)
      create(:activation, federation_object: object, activated_at: rand(old_activation.deactivated_at..Time.now.utc))
    end
    ...
    idps.each do |idp|
      [*core_attributes.reject { rand > 0.99 }, *optional_attributes.reject { rand > 0.8 }].each do |attr|
        create(:identity_provider_saml_attribute, identity_provider: idp, saml_attribute: attr)
      end
    end

    sps.each do |sp|
      [*core_attributes.reject { rand > 0.3 }, *optional_attributes.reject { rand > 0.05 }].each do |attr|
        optional = (rand > 0.7 && attr.core?) || (rand > 0.95)
        create(:service_provider_saml_attribute, service_provider: sp, saml_attribute: attr, optional: optional)
      end
    end
